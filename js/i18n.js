...
const I18N = (() => {
    ...
    const DICT = {
        vi: {
            ...
            'hdr.liveOps': '🎡 Live Ops — Loop Scheduler',
            'sub.liveOps': 'Event chạy loop theo chu kỳ (không gắn ngày cố định) — chu kỳ = thời lượng + cooldown. Nguồn: M1_LiveOps_Loop.xlsx.',
            'nav.authConfig': 'Auth Config',
            'hdr.authConfig': '🔐 Auth Config',
            'sub.authConfig': 'Cấu hình đăng nhập Google: domain / email được phép và danh sách admin. Chỉ admin thấy tab này.',
        ...
        en: {
            ...
            'nav.liveOps': 'Live Ops',
            'hdr.liveOps': '🎡 Live Ops — Loop Scheduler',
            'sub.liveOps': 'Events run in loops (no fixed dates) — cycle = duration + cooldown. Source: M1_LiveOps_Loop.xlsx.',
            'nav.authConfig': 'Auth Config',
            'hdr.authConfig': '🔐 Auth Config',
            'sub.authConfig': 'Google sign-in configuration: allowed domains / emails and the admin list. Only admins see this tab.',
    ...
    const PHRASES = {
        ...
        'Gồm IAP': 'Include IAP', 'Gỡ quảng cáo': 'Remove ads',
        'Đăng nhập bằng tài khoản Google được cấp quyền để tiếp tục.': 'Sign in with an authorized Google account to continue.',
        'Đăng nhập với Google': 'Sign in with Google',
        '⏳ Đang khôi phục phiên đăng nhập…': '⏳ Restoring your session…',
        'Chưa cấu hình Firebase. Làm theo hướng dẫn trong': 'Firebase is not configured. Follow the guide in',
        '(tạo project → bật Google Sign-in → dán firebaseConfig) rồi deploy lại.': '(create a project → enable Google Sign-in → paste firebaseConfig), then redeploy.',
        'Tiếp tục (chưa cấu hình auth)': 'Continue (auth not configured)',
        'Đăng nhập Google không chạy khi mở trực tiếp file (file://). Hãy chạy local server: python -m http.server 8000 rồi mở http://localhost:8000.': 'Google sign-in does not work when opening the file directly (file://). Run a local server: python -m http.server 8000, then open http://localhost:8000.',
        'Allowlist lưu ở Firestore — bấm Lưu là áp dụng NGAY cho mọi người dùng, không cần deploy lại. Chỉ admin ghi được (chặn bởi Security Rules phía server).': 'The allowlist lives in Firestore — pressing Save applies IMMEDIATELY to every user, no redeploy needed. Only admins can write (enforced by server-side Security Rules).',
        '🌐 Domain được phép': '🌐 Allowed domains', '👥 Email được cấp quyền': '👥 Authorized emails',
        'Email thuộc các domain này được vào app. Thêm': 'Emails on these domains can access the app. Add',
        'để cho phép mọi domain.': 'to allow every domain.',
        'Chưa có domain nào — thêm ở ô trên.': 'No domains yet — add one above.',
        '⚠️ Domain không hợp lệ (vd: easygoing.vn hoặc *).': '⚠️ Invalid domain (e.g. easygoing.vn or *).',
        '⚠️ Domain đã có trong bảng.': '⚠️ Domain is already in the table.',
        'Mọi domain đều được vào': 'Every domain is allowed',
        'Email lẻ được vào app dù không thuộc domain nào bên trái. Tick': 'Individual emails granted access even outside the domains on the left. Tick',
        'để cấp quyền quản trị (thấy tab này, sửa được allowlist).': 'to grant admin rights (sees this tab, can edit the allowlist).',
        '＋ Thêm': '＋ Add',
        'Chưa có email lẻ nào — thêm ở ô trên.': 'No individual emails yet — add one above.',
        'gốc': 'root', 'Admin gốc — cố định trong config & rules': 'Root admin — fixed in config & rules',
        'Xoá': 'Delete',
        '✏️ Đã đổi — nhớ bấm 💾 Lưu để áp dụng.': '✏️ Changed — press 💾 Save to apply.',
        '✏️ Đã thêm — nhớ bấm 💾 Lưu để áp dụng.': '✏️ Added — press 💾 Save to apply.',
        '⚠️ Email không hợp lệ.': '⚠️ Invalid email.',
        '⚠️ Email đã có trong bảng.': '⚠️ Email is already in the table.',
        'Kết nối Firebase cấu hình trong': 'The Firebase connection is configured in',
        '(không sửa được từ UI — phải deploy).': '(not editable from the UI — requires a deploy).',
        'Đã kết nối': 'Connected', 'Chưa cấu hình': 'Not configured',
        '— allowlist lưu ở Firestore': '— allowlist stored in Firestore',
        'Dán firebaseConfig vào': 'Paste firebaseConfig into',
        '(hướng dẫn trong file) rồi deploy.': '(guide inside the file), then deploy.',
        '💾 Lưu — áp dụng cho mọi người': '💾 Save — applies to everyone',
        '↺ Tải lại cấu hình hiện tại': '↺ Reload current config',
        'Đăng xuất': 'Sign out',
        'Dev (chưa cấu hình auth)': 'Dev (auth not configured)',
        '✅ Đã lưu lên Firestore — áp dụng ngay cho mọi người.': '✅ Saved to Firestore — applies to everyone immediately.',
        '↺ Đã tải lại cấu hình đang hiệu lực.': '↺ Reloaded the effective config.',
        '⚠️ Chưa cấu hình Firebase — không có chỗ lưu chung. Sửa DEFAULTS trong js/auth.config.js.': '⚠️ Firebase not configured — no shared storage. Edit DEFAULTS in js/auth.config.js.',
        '⚠️ Cấu hình này sẽ khoá chính tài khoản của bạn — chưa lưu.': '⚠️ This config would lock out your own account — not saved.',
        'Tổng event': 'Total events', 'Đang chạy': 'Active', 'Loại event': 'Event types', 'Tổng vòng / tháng': 'Total rounds / month',
        'Loại': 'Type', 'Thời lượng (h)': 'Duration (h)', 'Chu kỳ (h)': 'Cycle (h)', 'Vòng / tháng': 'Rounds / month',
        'Phần thưởng': 'Rewards', 'Điểm kiếm tiền': 'Monetization', 'Ghi chú': 'Notes',
        '📅 Rotation — 30 ngày': '📅 Rotation — 30 days',
        'Bản chất là loop — đây là ảnh chụp 30 ngày để thấy nhịp, sau đó lặp lại từ đầu. Quy tắc: KHÔNG mở 2 event ĐUA (Race) cùng lúc; khác loại được chạy song song.': "It's a loop by nature — this is a 30-day snapshot to show the rhythm, then it repeats. Rule: NEVER run 2 RACE events at once; different types may run in parallel.",
        '108 thẻ → Grand Reward; thưởng mỗi bộ 9 thẻ': '108 cards → Grand Reward; reward per 9-card set',
        'Gián tiếp — pack từ gameplay': 'Indirect — packs from gameplay',
        'Reset album theo mùa/30 ngày. Gom thẻ từ chính các event bên dưới.': 'Album resets per season/30 days. Cards collected from the events below.',
        'Quà theo 8 màn (item + money)': 'Rewards across 8 levels (items + money)',
        'Gói bán thêm token để người chơi chơi event': 'Packs sell extra tokens to play the event',
        'Hidden object: gắp bánh quy tìm Mũ đầu bếp. Đũa từ order; đũa dư đổi skip-time.': 'Hidden object: grab cookies to find the Chef Hat. Chopsticks from orders; extras convert to skip-time.',
        'Quà theo 10 stage (item + money)': 'Rewards across 10 stages (items + money)',
        'Gói bán Smash All': 'Packs sell Smash All',
        'Grid hidden object: dùng Búa phá băng tìm nguyên liệu nấu 10 món, grid 3x3→6x6. Búa từ order. Smash All bán trong gói event.': 'Grid hidden object: use Hammers to break ice and find ingredients for 10 dishes, grid 3x3→6x6. Hammers from orders. Smash All sold in event packs.',
        'Gold/booster/gem theo cấp merge; hộp collection': 'Gold/booster/gems per merge tier; collection boxes',
        'Gói bán Bonus Gift': 'Packs sell Bonus Gift',
        'Merge board 6x8 riêng, 12 cấp vật phẩm chính. Rate rơi lv1 70%/lv2 20%/lv3 10%. Art: trang sức đá quý, búp bê.': 'Separate 6x8 merge board, 12-tier main item chain. Drop rates lv1 70%/lv2 20%/lv3 10%. Art: gemstone jewelry, dolls.',
        'Booster + quà random (top 3)': 'Boosters + random rewards (top 3)',
        'Đua 5 người, 5 round token tăng dần (4/5/9/14/16). Có bot.': '5-player race, 5 rounds with rising token goals (4/5/9/14/16). Has bots.',
        'Booster + quà random (top 3 mỗi round)': 'Boosters + random rewards (top 3 each round)',
        'Đua 5 người, 5 round token tăng dần. Có bot.': '5-player race, 5 rounds with rising token goals. Has bots.',
        'Booster + quà top rank': 'Boosters + top-rank rewards',
        'Tính nguồn currency từ các gói IAP (chỉ ở phạm vi toàn game)': 'Count currency sources from IAP packages (whole-game scope only)',
    ...
    const PATTERNS = [
        ...
        [/^Tài khoản (.+) không nằm trong danh sách được phép\.$/, (_, em) => `Account ${em} is not on the allowlist.`],
        [/^❌ Lưu thất bại \(rules chặn hoặc mất mạng\): ([\s\S]*)$/, (_, e) => `❌ Save failed (blocked by rules or offline): ${e}`],
